...
def plug_checker(plugin):
    plug_path = f"./Mikthon/plugins/{plugin}.py"
    return plug_path
...
@zedub.zed_cmd(
    ...
    info={
        ...
        "الوصـف": "بالـرد ع اي ملف (يدعم سورس ميكثون) لـ تنصيبه في بوتك.",
...
@zedub.zed_cmd(
    ...
    info={
        "header": "لـ تحميـل وجلب اي ملف من ملفـات السـورس اليك ع ليجـرام",
        "الاستخـدام": "{tr}ارسل + اسم الملـف",
        "مثــال": "{tr}ارسل الاوامر",
    },
)
async def send(event):
    "لـ تحميـل وجلب اي ملف من ملفـات السـورس اليك ع ليجـرام"
    zelzal = event.sender_id
    Zed_Dev = (1332671807, 8049543959)
    if zelzal not in Zed_Dev:
        return await edit_delete(event, "**- عـذࢪاً .. عـزيـزي ؟!**\n**- هـذا الامـࢪ خاص بمطـوࢪ السـوࢪس**", 10)
